...
def build_report(train_path: Path, test_path: Path) -> dict:
    ...
    report["id_leakage_between_splits"] = {
        "count": len(inter_id),
        "examples": list(sorted(inter_id))[:5],
    }
    ...
    for col in ["LIMIT_BAL"]:
        check_range(col, ge=1, le=1_000_000)
    for col in ["AGE"]:
        check_range(col, ge=18, le=100)
    for col in [f"PAY_{k}" for k in [0, 2, 3, 4, 5, 6]]:
        check_range(col, ge=-2, le=9)
    for col in [f"BILL_AMT{k}" for k in range(1, 7)]:
        check_range(col, ge=-1_000_000, le=10_000_000)
    for col in [f"PAY_AMT{k}" for k in range(1, 7)]:
        check_range(col, ge=0, le=10_000_000)
    ...
    def unexpected_values(col, allowed):
        ...
        if diff:
            bad.append(
                {"column": col, "issue": "unexpected_categories", "values": diff}
            )
    ...
    report["mean_diff_num"] = {
        k: v
        for k, v in drift_simple.items()
        if v["relative_diff"] > 0.2 and abs(v["mean_train"]) > 1e-9
    }

    fail_reasons = []
    if (
        report["duplicates"]["train_rows_duplicated"] > 0
        or report["duplicates"]["test_rows_duplicated"] > 0
    ):
        fail_reasons.append("Row duplicates found")
    if (
        report["duplicates"]["train_id_duplicated"] > 0
        or report["duplicates"]["test_id_duplicated"] > 0
    ):
