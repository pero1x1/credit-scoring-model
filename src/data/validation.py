...
class CreditSchema(pa.SchemaModel):
    ...
    EDUCATION: pt.Series[int] = pa.Field(isin=[0,1,2,3,4,5,6], coerce=True)  # встречаются 0/5/6
...
def validate_csv(path: Path) -> dict:
    ...
    try:
        ...
        report = err.failure_cases[["column", "check", "failure_case"]].head(20).to_dict(orient="records")
        return {"ok": False, "errors": report}

def main(train_csv: str, test_csv: str, out_json: str = "data/processed/validation_report.json"):
    report = {
